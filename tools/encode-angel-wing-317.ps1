...
$candidates=@(); $cmd=Get-Command blender.exe -ErrorAction SilentlyContinue
...
$blender=$candidates[0]; $srcFull=(Resolve-Path -LiteralPath $inputObj).Path
$rawFull=[System.IO.Path]::GetFullPath($outRaw); $gzFull=[System.IO.Path]::GetFullPath($outGz)
...
WHITE_MID=126
WHITE_SHADE=125
GOLD_LIGHT=(6<<10)|(3<<7)|108
GOLD_DEEP=(6<<10)|(4<<7)|82
...
def pair(vs,fs):
    ...
    cx=(min(xs)+max(xs))/2;cy=(min(ys)+max(ys))/2;cz=(min(zs)+max(zs))/2;shift=(max(xs)-min(xs))*.53
...
def encode(vs,fs):
    ...
    cols=b''.join(struct.pack('>H',face_color(vs,f))for f in fs)
    body=bytes(vf)+bytes(ft)+bytes(ind)+cols+bytes(xs)+bytes(ys)+bytes(zs)
...
vs,fs=pair(vs,fs);q=[(round(x*SCALE),round(-y*SCALE)+PLAYER_Y_OFFSET,round(z*SCALE)+PLAYER_Z_OFFSET)for x,y,z in vs]
...
print('LEVINCIA_317_ENCODE_OK=1');print('MODEL_ID=100500');print('STYLE=WHITE_GOLD');print('SCALE=%s Y=%d Z=%d'%(SCALE,PLAYER_Y_OFFSET,PLAYER_Z_OFFSET));print('VERTICES=%d FACES=%d RAW_BYTES=%d GZIP_BYTES=%d'%(len(q),len(fs),len(raw),os.path.getsize(GZ)));print('RAW='+RAW);print('GZ='+GZ)
"@
