...
public final class ObjectDefinition {
	...
	public static ObjectDefinition forID(int id) {
		...
		if (definition.type == 4170) {
			definition.name = "Lunar Chest";
			// Custom model 22375 is not present in the current cache.
			// Keep the object's original cached model so the region can load.
			// definition.objectModelIDs = new int[]{22375};
			definition.actions = new String[]{"Open", null, null, null, null};
		}

		if (definition.type == 2153) {
			definition.name = "Soulpyre";
			definition.description = "A pyre specifically meant for burning souls".getBytes();
			definition.actions = new String[]{"Offer-souls", null, null, null, null};
		}

		if (definition.type == 26121) {
			definition.name = "Magic Beehive";
			definition.description = "The hum of life never rests.".getBytes();
			definition.actions = new String[]{"Harvest", null, null, null, null};
		}

		if (definition.type == 2816) {
			definition.name = "Arcane Crystal";
			definition.description = "The energy seeps slowly — if you’re patient.".getBytes();
			definition.actions = new String[]{"Mine", null, null, null, null};
		}
		...
		if (definition.type == 22721) {
			definition.interactive = true;
			definition.name = "<col=89584A>Soulfire forge";
			definition.actions = new String[]{"Make-offering", null, null, null, null};
		}
		if (definition.type == 12966) {
			// Custom model 22374 is not present in the current cache.
			// Keep the object's original cached model so the region can load.
			// definition.objectModelIDs = new int[]{22374};
			definition.name = "<col=89584A>Levincia Statue";
		...
		if (definition.type == 1105) {
			...
			definition.interactive = true;
			definition.solid = false;
			definition.name = "Levincia's Throne";
		}
		...
		if (definition.type == 41206) {
			...
			definition.name = "Isles of Levincia Chest";
			definition.actions = new String[]{"Open", null, null, null, null};
		}
		...
		if (definition.type == 6283) {
			definition.name = "Levincia Pillar";
		}
	...
	public static void loadEvilTree(ObjectDefinition definition) {
		switch (definition.type) {
			...
			case 11394:
				...
				definition.anIntArray776 = null;
				definition.name = "Whispering tree";
				definition.modifiedModelColors = new int[]{0};
				definition.originalModelColors = new int[]{1};
				definition.width = 3;
				definition.length = 3;
				...
				definition.brightness = 15;
				definition.contrast = 0;
				definition.actions = new String[]{"Listen-to",};
				definition.mapFunctionID = -1;
				definition.mapSceneID = -1;
				definition.aBoolean751 = false;
				definition.castsShadow = true;
...
*/
	...
	public boolean method579() {
		...
		boolean flag1 = true;
		for (int objectModelID : objectModelIDs) {
			int modelId = objectModelID & 0xffff;
			boolean loaded = Model.method463(modelId);
			if (!loaded) {
				System.out.println("[LEVINCIA MISSING OBJECT MODEL] objectId=" + type
						+ " objectName=" + (name == null ? "null" : name)
						+ " modelId=" + modelId);
			}
			flag1 &= loaded;
		}
