...
public class Client extends GameRenderer {
    ...
    private long levinciaLastLoadDebug = 0L;
  ...
  /*  public final void addParticle(Particle p) {
    ...
    public void handleHovers(boolean alertScreen) {
        ...
        if (!alertScreen) {
            ...
            if (mouseX >= 504 && mouseX <= 704 && mouseY >= 340 && mouseY <= 390) {
            ...
            if (showTwoFactorAuth) {
                ...
                if (mouseX >= 510 && mouseX <= 530 && mouseY >= 410 && mouseY <= 435) {
                    //cursor = Configuration.NEW_CURSORS ? 1061 : Cursor.HAND_CURSOR;
            ...
            if (mouseX >= 95 && mouseX <= 410) {
                if (mouseY >= 394 && mouseY <= 449) {
                    accountHovers[0] = mouseX >= 125 && mouseX <= 165;
                    accountHovers[1] = mouseX >= 225 && mouseX <= 265;
                    accountHovers[2] = mouseX >= 315 && mouseX <= 365;
            ...
            if (mouseX >= 152 && mouseX <= 420) {
                if (mouseY >= 394 && mouseY <= 404) {
                    accountDeletion[0] = mouseX >= 183 && mouseX <= 193;
                    accountDeletion[1] = mouseX >= 283 && mouseX <= 293;
                    accountDeletion[2] = mouseX >= 383 && mouseX <= 393;
    ...
    private final String bannerMessage = "Join the Levincia Realm and embark on a adventure with a wonderful community       Owner/Developer: Hank_rsps       All items in the store are currently 30% off During Beta!";
    ...
    public void drawLoginScreen(boolean flag) {
        ...
        // initialize banner start-X once
        if (bannerX == 0) {
            bannerX = getScreenWidth();
        }

        int centerX = getScreenWidth() / 2;
        int centerY = getScreenHeight() / 2;
        int loginOffsetX = centerX - 382;
        ...
        // ─── Fade-in phase ──────────────────────────────────────────────────────────
        if (titleAlpha < 250) {
            spritesMap.get(449).drawTransparentSprite(
                    centerX - spritesMap.get(449).myWidth / 2,
                    centerY - spritesMap.get(449).myHeight / 2,
                    titleAlpha
            );
            ...
            // draw background/logo full-screen
            spritesMap.get(449).drawAdvancedSprite(0, 0);

            // ─── scrolling announcement banner ────────────────────────────────────
            bannerX -= 2;
            int textW = chatTextDrawingArea.getTextWidth(bannerMessage);
            if (bannerX + textW < 0) {
                bannerX = getScreenWidth();
            }
            // bar behind text
            DrawingArea.drawFilledPixels(0, 10, getScreenWidth(), 20, 0x003355);
            // announcement text
            chatTextDrawingArea.drawRegularText(
                    true,              // drop shadow
                    bannerX,           // x
                    0x38C4FF,          // text color
                    bannerMessage,     // your announcement
                    10 + 20 - 6        // y (baseline)
            );
            // ───────────────────────────────────────────────────────────────────────

            // draw “Remember” checkbox text
            smallText.drawRegularText(true, loginOffsetX + 540, 0xFFFFFF, "Remember", 424);

            // ─── debug mouse coords ────────────────────────────────────────────────
           /* normalText.method385(
                    0xFFFFFF,
                    "X:" + super.mouseX + " Y:" + super.mouseY,
                    10, 20
            );*/

            // ─── “Save Accounts” toggle sprite ────────────────────────────────────
            if (!showTwoFactorAuth) {
                if (Configuration.SAVE_ACCOUNTS) {
                    spritesMap.get(452).drawAdvancedSprite(loginOffsetX + 511, 410);
                } else {
                    spritesMap.get(451).drawAdvancedSprite(loginOffsetX + 511, 410);
                }
            }
            ...
            if (showTwoFactorAuth) {
                if (textArea1Hover) {
                    spritesMap.get(470).drawAdvancedSprite(loginOffsetX + 237, 270);
                } else {
                    spritesMap.get(469).drawAdvancedSprite(loginOffsetX + 237, 270);
                }
                // blinking cursor
                if (loginScreenCursorPos == 0 && loopCycle % 45 < 10) {
                    chatTextDrawingArea.drawRegularText(true, loginOffsetX + 252, 0xFFFFFF, currentPinCode + "|", 292);
                } else {
                    chatTextDrawingArea.drawRegularText(true, loginOffsetX + 252, 0xFFFFFF, currentPinCode, 292);
                }
            ...
            // ─── Saved Account Slots ────────────────────────────────────────────────
            if (!showTwoFactorAuth) {
                int drawAccountX = loginOffsetX + 138;
                for (int i = 0; i < accountHovers.length; i++) {
                    ...
                    if (account == null) {
                        ...
                        newSmallFont.drawCenteredString(
                                account.getUsername(),
                                drawAccountX + 18, 457, 0xFFFFFF, 0
                        );
                    }
                    drawAccountX += 98;
                }
            ...
            if (!showTwoFactorAuth) {
                for (int i = 0; i < loginMessages.length; i++) {
                    ...
                    chatTextDrawingArea.drawRegularText(
                            true,
                            clientWidth / 2 - chatTextDrawingArea.getTextWidth(s) / 2,
                            0xFFFFFF,
                            s,
                            y
                    );
                ...
                // captcha box
                drawColorBox(0xFFFFFF, (clientWidth - captcha.myWidth) / 2, 185,
                        captcha.myWidth, captcha.myHeight + 10);
                captcha.drawAdvancedSprite((clientWidth - captcha.myWidth) / 2, 190);
                // “Back” button
                if (otherHover) {
                    newRegularFont.drawCenteredString("Back", loginOffsetX + 245, 205, 0xFFD700, 0);
                } else {
                    newRegularFont.drawCenteredString("Back", loginOffsetX + 245, 205, 0xFFFFFF, 0);
                }
            }
        }
...
//        }
  ...
  /*  private void renderParticles() {
    ...
    private void render() {
        ...
        if (loggedIn) {
            ...
            if (Configuration.FOG_ENABLED) {
                Rasterizer.drawFog(0x101820, 3800, 4300);
            } else {

                Rasterizer.drawFog(0x101820, 7900, 7900);
    ...
    private void levinciaLoadDebug(String message) {
        long now = System.currentTimeMillis();
        if (now - levinciaLastLoadDebug >= 1500L) {
            levinciaLastLoadDebug = now;
            System.out.println("[LEVINCIA LOAD DEBUG] " + message
                    + " | region=" + currentRegionX + "," + currentRegionY
                    + " base=" + regionBaseX + "," + regionBaseY
                    + " plane=" + plane
    ...
    private int getMapLoadingState() {
        ...
        if (!flag) {
            levinciaLoadDebug("OBJECT MODELS NOT READY (-3)");
            return -3;
        }
        if (aBoolean1080) {
            levinciaLoadDebug("WAITING FOR PLAYER UPDATE PACKET 81 (-4)");
            return -4;
        } else {
            levinciaLoadDebug("REGION READY - entering loadingStage 2");
            loadingStage = 2;
